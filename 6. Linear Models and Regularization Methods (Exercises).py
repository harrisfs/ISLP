...
x_poly_n = scaler.fit_transform(x_poly)
lassocv = LassoCV(cv=10) 
lassocv.fit(x_poly_n, Y)
print('best alpha: %.4f' %lassocv.alpha_)
...
ridgecv    = RidgeCV(alphas=alphas).fit(xtrain_n, ytrain) #LOOCV
ridge_pred = ridgecv.predict(xtest_n)
ridge_err  = mean_squared_error(ytest, ridge_pred) #1534640
ridge_coef = pd.Series(ridgecv.coef_, index = xtrain.columns)
print(ridge_coef)

# (d)
lassocv    = LassoCV(cv=len(ytrain)).fit(xtrain_n, ytrain) #LOOCV
lasso_pred = lassocv.predict(xtest_n)
